...
// Cancel order
router.put('/:orderId/cancel', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    const order = await Order.findById(req.params.orderId);
  ...
  try {
    ...
    if (search) {
      filter.$or = [
        ...
        { 'shippingAddress.name': new RegExp(search as string, 'i') },
        { 'shippingAddress.phone': new RegExp(search as string, 'i') }
      ];
    }
...
router.put('/admin/:orderId/status', authenticateToken, requireAdmin, async (req: AuthRequest, res: Response) => {
  try {
...
router.get('/admin/stats/overview', authenticateToken, requireAdmin, async (req: AuthRequest, res: Response) => {
  try {
